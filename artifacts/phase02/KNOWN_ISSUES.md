...
5. `ts-rs` 生成期间会提示其自身不解析 `serde(deny_unknown_fields)` 属性。Rust serde 仍严格执行该属性，Electron Main 也执行逐方法严格 payload 校验；generated contract check、unit、integration 和 E2E 均通过。
