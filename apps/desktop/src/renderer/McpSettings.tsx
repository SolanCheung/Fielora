...
const diagnosticLabels: Record<string, string> = {
  ...
  UNAVAILABLE_CREDENTIAL_UNSUPPORTED: '当前 Slice 不支持 MCP credential、env 或 header 配置。',
};
  ...
  return <div className="settings-section" data-testid="settings-mcp">
    ...
    <section className="settings-card settings-mcp-card">
      ...
      {catalog && catalog.connections.length > 0 && <div className="settings-mcp-list" data-testid="mcp-connection-list">{catalog.connections.map((connection) => <article key={connection.connection_id} data-testid={`mcp-connection-${connection.connection_id}`}>
        ...
        <div><em className="ready">定义有效 · executable 未检查</em><small>{connection.command_argument_count} 个参数 · 凭据不支持</small><small>Tools：仅在当前 AgentRun 激活后可用</small></div>
      </article>)}</div>}
    ...
    <p className="settings-mcp-security" data-testid="mcp-security-note">本 Slice 不支持 MCP credentials、env、headers 或远程连接。Server 只会在你为一个正在进行的 AgentRun 明确激活，并通过现有 PROCESS Policy / Approval 后启动；Run 结束即停止。</p>
  </div>;
}
