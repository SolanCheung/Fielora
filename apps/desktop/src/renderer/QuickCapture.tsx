...
export function QuickCapture(){
  ...
  return <div className="quick-capture"><button onClick={()=>void capture()} data-testid="quick-capture" title="捕获当前页面或选区到 Inbox">捕获</button>{status&&<span className="quick-capture-toast" role="status" data-testid="quick-capture-status">{status}</span>}</div>;
}
