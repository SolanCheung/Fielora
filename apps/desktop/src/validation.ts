...
export function validateResumeAgent(value:unknown):ResumeAgentRunRequest{const input=object(value);exact(input,['run_id','attachments']);return{run_id:id(input.run_id,'Agent run'),...(input.attachments==null?{}:{attachments:validateAgentAttachments(input.attachments)})};}
