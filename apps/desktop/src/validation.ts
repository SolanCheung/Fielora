...
export function validateStartAgent(value:unknown):StartAgentRunRequest{const input=object(value);exact(input,['field_id','conversation_id','provider_config_id','model_id','task','permission','max_steps']);if(!(input.model_id===null||typeof input.model_id==='string')||!(input.max_steps===null||Number.isSafeInteger(input.max_steps)&&Number(input.max_steps)>=1&&Number(input.max_steps)<=64))throw new Error('Invalid Agent run');return{field_id:id(input.field_id),conversation_id:id(input.conversation_id),provider_config_id:id(input.provider_config_id),model_id:input.model_id===null?null:boundedUnicodeText(input.model_id,256,1024,'model'),task:boundedUnicodeText(input.task,32768,32768,'Agent task'),permission:enumValue(input.permission,new Set(['READ_ONLY','REVIEW_CHANGES','FULL_CONTROL']),'Agent permission') as StartAgentRunRequest['permission'],max_steps:input.max_steps as number|null};}
