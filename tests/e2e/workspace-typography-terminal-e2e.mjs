...
try {
  ...
  const typography = await cdp.eval(`(()=>{const context=getComputedStyle(document.querySelector('.conversation-context-path'));const breadcrumb=getComputedStyle(document.querySelector('.right-dock-breadcrumb'));const code=getComputedStyle(document.querySelector('.dock-code-highlight'));const colors=[...document.querySelectorAll('.syntax-keyword,.syntax-string,.syntax-literal,.syntax-comment')].map((node)=>getComputedStyle(node).color);return{contextFont:context.fontFamily,breadcrumbFont:breadcrumb.fontFamily,contextSize:context.fontSize,breadcrumbSize:breadcrumb.fontSize,codeFont:code.fontFamily,codeSize:code.fontSize,colorCount:new Set(colors).size,lineNumbers:document.querySelectorAll('.dock-code-line > i').length};})()`);
