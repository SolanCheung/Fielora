...
try {
  ...
  const setup = await cdp.eval(`(async()=>{const provider=await window.fielora.provider.create({provider_kind:'OPENAI_COMPATIBLE',display_name:'Rich Result Fixture',base_url:'https://example.com/v1',default_model:'__fielora_agent_fixture__',custom_endpoint_acknowledged:true});await window.fielora.provider.storeCredential({provider_config_id:provider.id,secret:'fixture-secret'});const project=await window.fieloraTest.createProject({title:'Rich Result Project',goal:'Typed reference verification',root_path:${JSON.stringify(projectRoot)}});const conversation=await window.fielora.conversation.create({field_id:project.field_id,title:'Typed Result',provider_config_id:provider.id,model_id:'__fielora_agent_fixture__'});const saved=await window.fielora.reference.create({field_id:project.field_id,title:'Typed Reference Fixture',url:'https://example.com/fielora/typed-reference'});const message=await window.fielora.conversation.createMessage({conversation_id:conversation.id,role:'USER',content:'FIELORA_AGENT_FIXTURE_RICH_RESULT 展示可信结果引用。',status:'COMPLETED',provider_config_id:null,model_id:null,invocation_id:null,references:[]});const run=await window.fielora.agent.start({field_id:project.field_id,conversation_id:conversation.id,user_message_id:message.id,provider_config_id:provider.id,model_id:'__fielora_agent_fixture__',task:message.content,permission:'REVIEW_CHANGES',max_steps:8,attachments:[],active_work_surface:null});return{fieldId:project.field_id,conversationId:conversation.id,runId:run.id,referenceId:saved.resource.id};})()`);
  await wait(cdp, `window.fielora.agent.get({run_id:${JSON.stringify(setup.runId)}}).then((run)=>run.status==='COMPLETED')`);
  await wait(cdp, `window.fielora.conversation.listMessages({conversation_id:${JSON.stringify(setup.conversationId)}}).then((messages)=>messages.some((message)=>message.role==='ASSISTANT'&&message.references?.length===3))`);
  const durable = await cdp.eval(`window.fielora.conversation.listMessages({conversation_id:${JSON.stringify(setup.conversationId)}}).then((messages)=>{const result=messages.find((message)=>message.role==='ASSISTANT'&&message.references?.length===3);return{content:result.content,references:result.references};})`);
  assert.equal(durable.references.length, 3, JSON.stringify(durable));
  const durableIdentities = durable.references.map((reference) => ({ id: reference.id, kind: reference.target.kind }));
  assert.deepEqual(durableIdentities.map((reference) => reference.kind), ['PROJECT_FILE', 'CODE_RANGE', 'WEB_REFERENCE']);
  ...
  await cdp.eval(`window.fielora.conversation.createMessage({conversation_id:${JSON.stringify(setup.conversationId)},role:'ASSISTANT',content:'Plain Markdown cannot claim [trusted navigation](fielora-reference:resultref_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa) or [a local file](file:///C:/Windows/System32/config).',status:'COMPLETED',provider_config_id:null,model_id:null,invocation_id:null,references:[]})`);
  ...
  assert.deepEqual(await cdp.eval(`[...document.querySelectorAll('.markdown-typed-reference')].map((button)=>button.dataset.referenceId)`), durableIdentities.map((reference) => reference.id));
  assert.equal(await cdp.eval(`document.querySelectorAll('.markdown-reference-unavailable').length`), 1);
  assert.equal(await cdp.eval(`[...document.querySelectorAll('.markdown-reference-unavailable')].every((node)=>node.tagName==='SPAN')`), true);
  ...
  await wait(cdp, `window.fielora.conversation.listMessages({conversation_id:${JSON.stringify(setup.conversationId)}}).then((messages)=>messages.some((message)=>message.references?.length===3)).catch(()=>false)`, 15_000);
  const recovered = await cdp.eval(`window.fielora.conversation.listMessages({conversation_id:${JSON.stringify(setup.conversationId)}}).then((messages)=>messages.find((message)=>message.references?.length===3).references.map((reference)=>({id:reference.id,kind:reference.target.kind})))`);
  ...
  console.log('RICH_RESULT_TYPED_REFERENCE_E2E: PASS');
} finally {
