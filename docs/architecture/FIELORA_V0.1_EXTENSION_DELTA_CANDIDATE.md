...
**Implementation:** `EXTERNAL TOOL PROVIDER PIPELINE PROBE: IMPLEMENTED / VALIDATED`; `PROJECT AGENT SKILL DISCOVERY + LAZY LOAD: IMPLEMENTED / TARGETED VALIDATION PASS`
...
| Skill discovery | `PARTIAL` | `BUILTIN_SKILLS`; `SkillCatalog::discover`; existing `list_skills` Tool | Harness.Ingress & Context + Harness.Orchestration | Built-ins plus bounded standard-format project Skills under `.agents/skills/<name>/SKILL.md` share one Run catalog projection with source/trust/digest facts. User/community roots, dependency resolution, search/ranking, and installer flows remain absent. |
| Skill lazy loading | `EXISTS` | Existing `load_skill`; `SkillCatalog::load_skill`; `ContextCompiler::admit_skill` | Harness.Ingress & Context + Harness.Orchestration | Tier-1 metadata is catalogued before selection; Tier-2 body is admitted only after `load_skill` and discovery/load digest equality. Tier-3 content loading and dependency handling remain absent. |
| Project/local Skill support | `PARTIAL` | `crates/fielora-agent/src/skills.rs`; Run-scoped catalog in `AgentCoordinator`; existing ToolCall receipt/event/context snapshot path | Harness.Ingress & Context | Project `.agents/skills` is implemented with exact filename, bounds, canonical containment, collision rejection and untrusted context marking. User-local/community sources and cross-scope precedence remain absent. |
...
| `EXTEND` | built-in Skill index/load | The first Slice now adds bounded compatible Project `SKILL.md` discovery, metadata-first projection, content digest, source/trust, lazy `ContextCompiler` admission and resource path inventory. User/local/community sources and dependency declarations remain later deltas. |
