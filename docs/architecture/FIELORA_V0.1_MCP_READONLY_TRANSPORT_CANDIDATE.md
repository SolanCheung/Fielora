...
| `ToolProvider` discovery compatibility | `EXISTS` | `ToolProvider::discover_tools`; `ProviderToolDefinition`; `coding_tool_catalog_with_providers` | Tools provides definitions; Harness controls exposure/selection | The seam accepts bounded definitions with a trusted Fielora adapter-authored existing `AgentToolEffect`. The MCP adapter continues to force untrusted `tools/list` contributions to `OBSERVE`; Web adapters can author existing `NETWORK`. The seam itself is not replaced. |
